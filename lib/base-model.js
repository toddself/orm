...
const RELATIONSQL = `INSERT INTO %I (relation, target_id, owner_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`
...
class BaseModel {
  ...
  _init (fields, data) {
    ...
    Object.keys(fields).forEach((prop) => {
      ...
      if (field._isRelation && field._relationType === 'many') {
  ...
  async load (opts) {
    ...
    const rels = Object.values(this._fields)
      .filter((field) => field._isRelation)
      .map((field) => format(SELECTIONRELATIONSQL, this._orm.getRelTable(this, field)))
    // @TODO: need to relate each relation field with its query
    const promises = [this._orm.query(dataSQL, [this.id])]
      .concat(rels)
    const [data, ...relations] = await Promise.all(promises)
    ...
    if (relations.length > 0) {
      // need the relation field, the field key and the results here for each row
      await Promise.all(relations.map((relation) => this.processRelation(relation)))
    }
    this._new = false
  }

  async _processRelation (relation) {
    // @TODO use relation field to figure out what model to load and what it's called
    // and the local field key to know where to assign it
    await Promise.all(relation.rows.map((row) => {
      debug('Processing row %o', row)
      const Model = this._orm._modelCache.get()
      if (!Model) {
