...
class BaseModel {
  ...
  async save () {
    ...
    const changes = this._changes.reconcile()
    const { simple, complex, relations } = parseChanges(changes, this._fields, this._data)
    let queries = []
    if (this._new) {
