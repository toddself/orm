...
const RELATIONSQL = `INSERT INTO %I SET (t elation, target_type, target_id, owner_id) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING`
  ...
  _define (key) {
    ...
    Object.defineProperty(this, key, {
      ...
      set: (val) => {
        ...
        if (key === this._idColumn) this._new = false
    ...
    for (const [key, relation] of Object.entries(relations)) {
      if (Array.isArray(relation)) {
        relation.forEach((rel) => {
          queries.push({ query: relationSQL, args: [key, rel.type, rel[rel._idColumn], this[this._idColumn]] })
        })
      } else {
        queries.push({ query: relationSQL, args: [key, relation.type, relation[relation._idColumn], this[this._idColumn]] })
      }
    }

    try {
      this._orm.transaction(queries)
    } catch (err) {
      throw err
    } finally {
